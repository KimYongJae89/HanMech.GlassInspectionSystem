...
{
    public class BottomBroken : BrokenDetection
    {
                    ...
                    if (cnt != 0)
                    {
                        if (Math.Abs(startXEdgeOneDerivativeArray[i]) == 0)
                        {
                            if (defectStartIndex >= 0)
                            {
                                ...
                                //Rectangle defect = BrokenDetection.GetDefectRoi(startX, startY, endX, endY, eEdgeType.Bottom, imageHeight / 2);
                                //Rectangle defect = new Rectangle(startX, startY, width, height);

                                //result.Add(defect);

                                if (startY >= (int)prev - minSize)
                                {

                                    Rectangle defect = new Rectangle(startX, startY - height, width, height);
                                    result.Add(defect);
                                }
                                else
                                {
